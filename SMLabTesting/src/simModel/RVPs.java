...
class RVPs {
	...
	protected double[] FAILURE_MEAN_ARR = new double[] { 840, -1, 540, 900, 960 };
	protected double[] REPAIR_TIME_MEAN_ARR = new double[] { 11, -1, 7, 14, 13 };// 去年11*60，
	// 且11和7中间加了-1//
	...
	protected RVPs(Seeds sd) {
		...
		timeToFailureDist = new Exponential((1.0 / FAILURE_MEAN_ARR[0]), new MersenneTwister(sd.timeToFailureSeeds));
	...
	protected double duSampleArr() {
		...
		System.out.println(model.getClock() + sampleArrDist.nextDouble(mean));
	...
	protected double uTimeToFailure(int cid) {
		...
		return timeToFailureDist.nextDouble(mean);
	}
	...
	protected double uRepairTime(int cid) {
		...
		return repairTimeDist.nextDouble(mean);
	}
	...
	protected int[] uGetSequence() {
		...
		if (value < 9) {
			sequence = new int[] { Constants.C1, Constants.C2, Constants.C4, Constants.C5, Constants.LA };
		} else if (value < 22) {
			sequence = new int[] { Constants.C3, Constants.C4, Constants.C5, Constants.LA };
		} else if (value < 37) {
			sequence = new int[] { Constants.C1, Constants.C2, Constants.C3, Constants.C4, Constants.LA };
		} else if (value < 49) {
			sequence = new int[] { Constants.C4, Constants.C3, Constants.C2, Constants.LA };
		} else if (value < 56) {
			sequence = new int[] { Constants.C2, Constants.C5, Constants.C1, Constants.LA };
		} else if (value < 67) {
			sequence = new int[] { Constants.C4, Constants.C5, Constants.C2, Constants.C3, Constants.LA };
		} else if (value < 81) {
			sequence = new int[] { Constants.C1, Constants.C5, Constants.C3, Constants.C4, Constants.LA };
		} else if (value < 87) {
			sequence = new int[] { Constants.C5, Constants.C3, Constants.C1, Constants.LA };
		} else {
			sequence = new int[] { Constants.C2, Constants.C4, Constants.C5, Constants.LA };
		}
		return sequence;
	}
